...
3. [ ] Move allowed transitions into a pure reducer or equivalent pure transition function. Setup/entry, consent, act-entry/break, and compiled question destinations are migrated and characterized in `src/closer/engine/transitions.js`; private resolution, finale, and global families remain in the controller.
