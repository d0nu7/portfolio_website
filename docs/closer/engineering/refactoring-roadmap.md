...
3. [ ] Move allowed transitions into a pure reducer or equivalent pure transition function. Setup/entry, consent, and compiled question destinations are migrated and characterized in `src/closer/engine/transitions.js`; act-entry/break, private resolution, finale, and global families remain in the controller.
