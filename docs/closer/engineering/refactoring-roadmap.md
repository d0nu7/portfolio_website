...
4. [ ] Keep rendering declarative: phase selectors decide which screen is shown; screen components emit events. Setup renders through `CloserSetupView.js`, and both private consent sequences share `CloserConsentView.js`; acts, private moments, finale, and question presentation remain to be split incrementally.
