...
2. [ ] Define explicit events such as `START_RUN`, `ANSWER_DONE`, `PASS`, `END_ACT`, `CONFIRM_CONSENT`, `END_RUN`, and `RESUME`. The frozen event inventory lives in the [transition matrix](transition-matrix.md); setup, consent, acts, private moments, finale, language, timer, and end-run events are implemented, while question-completion events remain incremental.
3. [ ] Move allowed transitions into a pure reducer or equivalent pure transition function. Setup/entry, consent, act-entry/break, private-moment capture/resolution, final-question reveal, Question 37, end-run, language, timer, and compiled question destinations are migrated and characterized in `src/closer/engine/transitions.js`. Canonical state creation, restart, resume preparation, and save parsing are extracted to `src/closer/engine/persistence.js`, and guarded browser storage lives in `src/closer/infrastructure/storage.js`; screen-local reset effects and Late Night discovery preferences remain in the controller.
